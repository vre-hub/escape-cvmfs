...
# Install reana-client into a pyenv virtualenv and consolidate site-packages
run_install () {
  local version="$1"
  ...
  pip install "reana-client==${REANA_VERSION}"
...
# Prepare workspace
BUILD_ROOT="$SCRIPT_DIR/$REANA_VERSION"
mkdir -p "$BUILD_ROOT"
cd "$BUILD_ROOT"
mkdir -p lib

echo "=== Building reana-client $REANA_VERSION ==="
...
mkdir -p bin
cp "$(pyenv virtualenv-prefix)/envs/${EXPORT_ENV_NAME}-py${BASE_PYTHON_VERSION}/bin/reana-client" bin/
...
for script in bin/*; do
  ...
  tmpfile="$(mktemp)"
  {
    printf '#!/bin/bash\n'
    printf '# -*- coding: utf-8 -*-\n'
    printf '"exec" "$REANA_PYTHONBIN" "-u" "-Wignore" "$0" "$@"\n'
...
tar zcf "$SCRIPT_DIR/reana-client-${REANA_VERSION}.tar.gz" *
echo " - $(ls -la "$SCRIPT_DIR/reana-client-${REANA_VERSION}.tar.gz")"
