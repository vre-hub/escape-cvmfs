...
# ---------------------------------------------------------------------------
# Scientific/binary deps we deliberately do NOT re-install — they come from the
# LCG view at runtime. Re-pip-installing them would shadow the view with a
# possibly ABI-incompatible numpy/pandas/etc. and break `import ROOT`.
#
# Pure-Python MCP-layer deps that LCG 107 doesn't ship are pip-installed
# explicitly below so the server actually works. Keep this list in sync with
# root-mcp's pyproject.toml (non-scientific rows only).
BRIDGE_DEPS=(
  "mcp>=0.9.0"
...
EXTRA=""
if $XROOTD; then
  # XRootD itself is already in LCG 107 (fsspec-xrootd, xrootd bindings), so
  # passing [xrootd] here only adds the Python shim — still --no-deps safe.
  EXTRA="[xrootd]"
fi

pip_install_target () {
  # $1 : pip spec (possibly with extra)
  # $2 : mode label for logs
  local spec="$1"
  local label="$2"
  echo "Installing ${spec} (${label}, --no-deps) into staging using ${BUILD_PYTHON}..."
  ...
  echo "Installing ${#BRIDGE_DEPS[@]} bridge deps (MCP layer not in LCG view)..."
  "$BUILD_PYTHON" -m pip install \
    --target "$SITE_PACKAGES" \
    --no-deps \
    --upgrade \
...
#
# Sources an LCG view so PyROOT, uproot, numpy, scipy, matplotlib, XRootD,
# etc. come from CVMFS. The root-mcp / root-cli site-packages dir (installed
# next to this wrapper) is prepended to PYTHONPATH so it wins over any
# same-named modules in the view.
set -e
